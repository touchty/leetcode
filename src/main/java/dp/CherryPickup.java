...
public class CherryPickup {
    public int cherryPickup(int[][] grid) {
        ...
        // n: length of the total steps from (0, 0) to any point of the grid.
        // # of rows + # of columns == n, which form a line.
        // i ((i, j)) and p ((p, q)) are two paths from (0, 0) to a grid with steps of  n
        for (int n = 1; n < M; n++) {
            for (int i = N - 1; i >= 0; i--) {
                for (int p = N - 1; p >= 0; p--) {
                    int j = n - i, q = n - p;
